...
export class CalendarExporter {
  ...
  private parseData(): void {
    ...
    while (true) {
      if (courseLoopCount++ > questConfig.MAX_COURSES) {
      ...
      while (true) {
        if (sectionLoopCount++ > questConfig.MAX_SECTIONS) {
    ...
    if (courseLoopCount === 0 || courseLoopCount >= questConfig.MAX_COURSES) {
